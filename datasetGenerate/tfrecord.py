...
directory='/Users/lulei05/Documents/study/deep_learning/common_data/第4章 配套资源/man_woman/'                                                     #定义样本路径
(filenames,labels),_ = load_sample(directory,shuffleflag=False)   #载入文件名称与标签


def makeTFRec(filenames,labels): #定义函数生成TFRecord
    writer= tf.python_io.TFRecordWriter(directory+"mydata.tfrecords") #通过tf.python_io.TFRecordWriter 写入到TFRecords文件
...
#############################################################
TFRecordfilenames = [directory + "mydata.tfrecords"]
saveimgpath = directory + 'show/'    #定义保存图片路径
#############################################################
def test_demo():
    #训练方式
